...
from django.conf.settings import POKEAPI_TYPES_URL
...
class AddUserToTypeGroupView(APIView):
    ...
    def post(self, request, type):
        if not type_exists_in_pokeapi(type):
            return Response({"detail": f"Type '{type}' is invalid."}, status=status.HTTP_400_BAD_REQUEST)

        group, _ = Group.objects.get_or_create(name=type)
        request.user.groups.add(group)
        return Response({"detail": f"User added to group '{type}'."}, status=status.HTTP_200_OK)
...
class RemoveUserFromTypeGroupView(APIView):
    ...
    def post(self, request, type):
        try:
            ...
            return Response({"detail": f"Group '{type}' does not exist."}, status=status.HTTP_404_NOT_FOUND)

        request.user.groups.remove(group)
        return Response({"detail": f"User removed from group '{type}'."}, status=status.HTTP_200_OK)
